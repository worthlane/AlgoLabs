...
Из графика можем сделать вывод, что из всех квадратичных сортировок ```ShellSort``` является самой быстрой. Причем заметно, что она намного быстрее остальных. Это все благодаря последовательности Кнутта. В отличие от других квадратичных сортировок, которые в худшем случае работают за $O(N^2)$, сортировка Шелла, использующая последовательность Кнутта работает в среднем за $O(N^{\frac{4}{3}})$, а в худшем случае за $O(N^{\frac{3}{2}})$.
...
Из графика можно понять, что реализованная нами быстрая сортировка по скорости почти сравнима со стандартным ```qsort```. Можно найти множество причин, почему стандартная сортировка оказалась быстрее. Я предполагаю, что это связано с тем, что стандартная сортировка может быть специально оптимизирована для лучшей работы с хешем и упрощения инструкций для компилятора. Также, вероятно, она может лучше чем мы обработать случай для сортировки малых подмассивов.

Также стоит отметить, что по скорости сортировки ```LSD sort``` фактически не имеет конкурентов. Он сортирует в разы быстрее любой из других сортировок рассмотренных нами. Это возможно отчасти благодаря тому, что в ```LSD sort``` нет рекурсии и крайне затратных внутренних циклов, в отличие от ```MSD sort```. Эта сортировка сравнивает байты чисел, в связи с чем у нее появляется $2^8 - 1 = 255$ веток рекурсии. Это заметно замедляет ее на фоне других сортировок при обработке массивов малой длины, но на длинных массивах она уступает лишь быстрой сортировке и ```LSD``` сортировке. Можно сказать, что график подтверждает теоретические данные о скорости сортировок:
